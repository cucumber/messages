      ...
      defp encode_file_name(acc, msg) do
        try do
          if msg.file_name == "" do
            ...
            [acc, "\x1A", Protox.Encode.encode_string(msg.file_name)]
          end
        rescue
          ArgumentError ->
            reraise Protox.EncodingError.new(:file_name, "invalid field value"), __STACKTRACE__
        end
      end
    ]
  ...
  (
    (
      ...
      (
        ...
        def decode!(bytes) do
          parse_key_value(bytes, struct(CucumberMessages.SourceReference.JavaStackTraceElement))
        end
      )
    )
        ...
        {field, rest} =
          case Protox.Decode.parse_key(bytes) do
            ...
            {3, _, bytes} ->
              {len, bytes} = Protox.Varint.decode(bytes)
              {delimited, rest} = Protox.Decode.parse_delimited(bytes, len)
              {[file_name: Protox.Decode.validate_string!(delimited)], rest}
  ...
  (
    ...
    def json_decode!(input, opts \\ []) do
      ...
      Protox.JsonDecode.decode!(
        input,
        CucumberMessages.SourceReference.JavaStackTraceElement,
