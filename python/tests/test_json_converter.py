...
class SimpleModel(JSONDataclassMixin):
    ...
    enum_field: SimpleEnum  # required
...
def test_simple_serialization():
    model = SimpleModel(
        string_field="test", int_field=42, float_field=3.14, bool_field=True, enum_field=SimpleEnum.VALUE1
    )
...
def test_simple_deserialization():
    ...
    assert model.enum_field == SimpleEnum.VALUE1
    assert model.optional_field == "optional"


def test_nested_serialization():
    simple = SimpleModel(
        string_field="nested", int_field=42, float_field=3.14, bool_field=True, enum_field=SimpleEnum.VALUE1
    )
...
def test_collections_serialization():
    simple = SimpleModel(
        string_field="item", int_field=42, float_field=3.14, bool_field=True, enum_field=SimpleEnum.VALUE1
    )

    model = CollectionsModel(sequence_field=["a", "b", "c"], list_field=[simple], dict_field={"key": simple})
...
def test_collections_deserialization():
    data = {
        "sequenceField": ["a", "b", "c"],
        "listField": [
            {"stringField": "item", "intField": 42, "floatField": 3.14, "boolField": True, "enumField": "value1"}
        ],
        "dictField": {
            "key": {"stringField": "item", "intField": 42, "floatField": 3.14, "boolField": True, "enumField": "value1"}
        },
    }
...
def test_datetime_deserialization():
    data = {"datetimeField": "2024-01-01T12:00:00", "dateField": "2024-01-01"}
...
def test_error_handling():
    ...
    with pytest.raises(TypeError, match="Error decoding field enumField"):
        SimpleModel.from_dict(
            {"stringField": "test", "intField": 42, "floatField": 3.14, "boolField": True, "enumField": "invalid_value"}
        )

    # Test missing required fields with only optional field
    with pytest.raises(TypeError, match="Missing required arguments:"):
        SimpleModel.from_dict({"optionalField": "only optional field provided"})
...
def test_invalid_field_names():
    ...
    with pytest.raises(
        TypeError, match="Missing required arguments: string_field, int_field, float_field, bool_field, enum_field"
    ):
